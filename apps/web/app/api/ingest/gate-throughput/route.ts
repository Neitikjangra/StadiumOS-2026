...
import { GateThroughputPayload } from "@/lib/events/schemas";

export async function POST(request: NextRequest) {
  const body = await request.json();
  const result = GateThroughputPayload.safeParse(body);
  if (!result.success) {
  ...
  const key = `gate_throughput:${stadiumId}:${result.data.gateId}:${Date.now()}`;
  const job = await ingestionQueue.add("ingest", {
    channel: "gate_throughput",
    stadiumId,
    sourceId: result.data.gateId,
    payload: result.data,
    idempotencyKey: key,
  });
